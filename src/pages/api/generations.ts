...
export async function POST({ request, locals }: APIContext) {
  ...
  try {
    ...
    if (error || !data) {
      return new Response(JSON.stringify({ error: error || "Nieprawidłowe dane wejściowe" }), {
        status: 400,
    ...
    const generationService = new GenerationService(supabase, import.meta.env.OPENROUTER_API_KEY);
